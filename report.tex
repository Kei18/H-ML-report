...
AdaGrad を使用した場合, 更新式は次のようになる.
\begin{align*}
 \bm{w}^{(t)}
 &= \argmin_{\bm{w}}\left((\bm{w} - \bm{w}^{(t-1)})^\mathrm{T}
 \left(\sum_{t}\bm{g}_t\right)+\lambda||\bm{w}||_1
 +\frac{1}{2\eta_0}||\bm{w}-\bm{w}^{(t-1)}||^2_{\bm{H}_t}\right)\\
 &= ST_{\eta_0\frac{\lambda}{t}}\left(\bm{w}^{(t-1)}-\eta_0\bm{H}^{-1}_t
 \left(\sum_{t}\bm{g}_t\right)\right)\\
\end{align*}
